...
import {
	...
	DefaultCSATName,
	TrackEventsPublishAPIMethodPath,
...
export class PostSurvey {
	...
	async PostSurvey(event: any) {
		console.log(`Creating survey on event`, event.payload);
		try {
			...
			if (payload.hasOwnProperty('command_id')) {
				...
				if (objId.includes("ticket")) {
					...
					console.log(`Work information for object ID: [${objId}] ${work}`);
					objType = work.type;
					...
					console.log(`conversation information for object ID: [${objId}] ${conversation}`);
			...
			for (let i = 0; i < surveyStakeholders.length; i++) {
				...
				if (surveySentOnChannels.length > 0) {
					isSurveySent = true;
					// Send survey dispatch event on blubox.
					const uuidWithoutDon = getUuidV4Pattern(deferredUUID)
					// In case email is not defined this will still add email in the survey channels.
					const trackEvent = trackEventsBody(devOid, objId, uuidWithoutDon, deferredUUID,
						surveyStakeholders[i].id!, surveyId, surveySentOnChannels)
					const resp = await doDevRevPostAPICall(apiBase, TrackEventsPublishAPIMethodPath, trackEvent, auth)
					if (resp.ok) {
						console.log(`Successfully published [survey_dispatch] event with event_id [${uuidWithoutDon}].`);
					} else {
						let body = await resp.text();
						console.error(`Error while publishing [survey_dispatch] event with event_id [${uuidWithoutDon}].`, resp.status, body);
					}
				}
			}
